...
write_compose_files() {
  ...
  # IMPORTANT: we do NOT ask for broker keys here.
  # Broker keys are provisioned from the mobile app via encrypted command SET_BROKER_KEYS.
  cat > "$BOT_DIR/.env" <<EOF
...
first_time_setup_interactive() {
  ...
  echo ""
  echo "Next step (from mobile app): send broker keys using command SET_BROKER_KEYS."
  echo "After keys are saved, the bot will automatically start trading."
}
